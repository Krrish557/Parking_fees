...
3-24 hours     $89\n\n''')
        ...
        # Calculate total parking time correctly
        hour_diff = ch_out_h - ch_in_h
        minute_diff = ch_out_m - ch_in_m

        if minute_diff < 0:  # Borrow an hour if minutes go negative
            hour_diff -= 1
            minute_diff += 60

        print(f"** Total parking time: {hour_diff} hours and {minute_diff} minutes")

        # Determine fee based on the correct parking time
        ...
        while total_collected < cash_ask:
            cash_submitted = int(input(cash_ask_text))
            total_collected += cash_submitted  # Update total collected amount
            ...
            if total_collected >= cash_ask:
                ...
                if change > 0:
                    ...
                    input("** Please collect your change. Press ENTER when done...")
